...
public class RDPlayer {
	...
	public void save() {
		if (RDPlayers.saveData == true) {
			try {
				...
				RankupOnKillsPlugin.debug("Saved YAML file: " + playerFile);
			} catch (IOException ex) {
				Bukkit.getServer().getLogger().log(Level.SEVERE, "Could not save config to "
						+ playerFile, 
						ex);
			}
		}
	...
	public void load() {
		playerFile = new File("plugins/"+RDPlayers.plugin.getName()+"/players/", name + ".yml");
		player = YamlConfiguration.loadConfiguration(playerFile);
		RankupOnKillsPlugin.debug("Loaded YAML file: " + playerFile);
